...
| Verify screenshot path | Stream result adapters in the demo | Screenshot and optional `Read` verification paths printed in summary | `ClaudeCodeToolResultAdapter` extracts `Screenshot saved: /home/oai/share/output/screenshots/<file>` | `GhcToolExecutionAdapter` extracts the same path from successful `tool.execution_complete` content |
